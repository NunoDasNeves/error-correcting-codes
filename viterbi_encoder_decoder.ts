...
class BitArray {
    ...
    // push bit on the end of the array
    ...
    pushBit(bit: number) {
        ...
        // get indices
        const lastArrayIndex = Math.trunc((this.length - 1)/BitArray.BITS_PER_NUM)
    ...
    // remove last bit from array and return it
    popBit(): number {
        if (length == 0) throw Error("Can't pop empty BitArray")

        // get index of last number
        const lastArrayIndex = Math.trunc((this.length - 1)/BitArray.BITS_PER_NUM)
        const bitIndex = this.length - 1 - lastArrayIndex * BitArray.BITS_PER_NUM
        // get and clean the bit
        const bit: number = (this.array[lastArrayIndex] >> bitIndex) & 1
        // reduce length of array if popping from the first bit
        if (bitIndex == 0 && lastArrayIndex > 0) this.array.pop()
        this.length--
    ...
    leftShift() {
        ...
        for (let i: number = this.array.length - 1; i >= 0; --i) {
            if (i < this.array.length - 1) {
                // get bit that will be left shifted away
                const bit: number = (this.array[i] >> (BitArray.BITS_PER_NUM - 1)) & 1
                // put it into next number
                // clear and set rightmost bit
                this.array[i+1] &= ~1
                this.array[i+1] |= bit
            }
            this.array[i] = this.array[i] << 1
        }
        // clear rightmost bit in entire BitArray
        this.array[0] &= ~1
...
class Decoder {
    ...
    gen: BitArray[]     // n binary generator polynomials of length K
    input: BitArray     // string of n-bit symbols generated by the encoder (length should be a multiple of n)

    // state transition graph, indexed by state number
    // in the decoder, the state is just an integer in [0, N)
    // { string: { prev: [ string, string ], output: [ BitArray, BitArray ] }, ... }
    // string key is a BitArray as a string of 1's and 0's
    // prev contains the previous possible states in no particular order, as strings
    ...
    // (input.length/n) table of [ { string: { hamming: number, prev: string, bit: number }, ... }, ... ]
    // The string key is a BitArray as a string of 1's and 0's
    // hamming is the culmulative best hamming distance to this node
    // prev is the previous state as another string
    ...
    constructor(n: number, K: number, gen: BitArray[], input: BitArray) {
        ...
        // initialise table for DP
        this.table = new Array<any[]>()
        this.table.push({})
        // we always start in state 0 with 0 accumulated hamming distance
        this.table[0][BitArray.fromNumber(0, K-1).toString()] = { hamming: 0, prev: 'invalid', bit: -1 }
        // fill rest of row with impossible states
        for (let s: number = 1; s < this.N; ++s) {
            this.table[0][BitArray.fromNumber(s, K-1).toString()] = { hamming: Number.MAX_SAFE_INTEGER, prev: 'invalid', bit: -1 }
        }

        // initialise state transition graph for looking up states and outputs
        this.graph = {}
        // for each state
        for (let s: number = 0; s < this.N; ++s) {
            // encoder representation of state
            const state: BitArray = BitArray.fromNumber(s, K-1)
            // rightmost bit = 0
            const prev_1: BitArray = BitArray.from(state).leftShift()
            prev_1.popBit()
            // rightmost bit = 1
            const prev_2: BitArray = BitArray.from(prev_1).setBit(0, 1)
    ...
    // input is 1 or 0
    get_output(input: number, state: BitArray) {
        const full_state: BitArray = BitArray.from(state).pushBit(input)
        // compute output bits
        const out_symbol: BitArray = BitArray.fromNumber(0, this.n)
        // for each output bit
        for (let j: number = 0; j < this.n; ++j) {
    ...
    static example(input: BitArray) {
        const poly_1: BitArray = BitArray.fromNumber(7, 3)
        const poly_2: BitArray = BitArray.fromNumber(5, 3)
        return new Decoder(2, 3, [poly_1, poly_2], input)
    }

    // hamming distance between two bit arrays of the same length
    // this acts as a likelihood function of observable symbol a being produced by actual symbol b
    hamming(a: BitArray, b: BitArray) {
        let dist: number = 0
        for (let i: number = 0; i < a.length; ++i) {
            // add the xor of each bit
            dist += a.getBit(i) ^ b.getBit(i)
        }
        return dist
    }
