...
export class Decoder {
  ...
  // current state
  i: number = -2        // the index of the next encoded symbol in the input array. i/2 = table.length-1
  ...
  next(): boolean {
    ...
    // return early
    if (this.i + this.n >= this.input.length) return false
    ...
    if (this.i + this.n >= this.input.length) {
      this.finished = true
      return false
    }
